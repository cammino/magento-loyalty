...
class Cammino_Loyalty_Model_Sales_Quote_Address_Total_Tax extends Mage_Sales_Model_Quote_Address_Total_Abstract
{
    ...
    public function collect(Mage_Sales_Model_Quote_Address $address)
    {
        ...
        try {
            $methodCode = '';
            if ((!empty($quote->getPayment())) && (!empty($quote->getPayment()->getMethodInstance()))) {
                $methodCode = $quote->getPayment()->getMethodInstance()->getCode();
            }
            if (($discount < 0) && (strpos($helper->getDiscountPaymentMethods(), $methodCode) !== false)) {
