            ...
            if($code != "loyalty") {
                ...
                if ($total > 0) {
                    ...
                    $data = array(
                        ...
                        "direction"         => 'credit',
                        "amount"            => ($points / $helper->getPointToMoney($order->getStoreId())),
                        "points"            => $points,
                        "money_to_point"    => $helper->getMoneyToPoint($order->getStoreId()),
                        "point_to_money"    => $helper->getPointToMoney($order->getStoreId()),
                        "status"            => 'pending',
        ...
        try {
            ...
            if($code == "loyalty" || $helper->hasLoyaltyDiscountApplied()) {
                ...
                $data = array(
                    ...
                    "order_id"          => $order->getId(),
                    "direction"         => 'debit',
                    "amount"            => (($points / $helper->getPointToMoney($order->getStoreId())) * -1),
                    "points"            => $points * -1,
                    "money_to_point"    => $helper->getMoneyToPoint($order->getStoreId()),
                    "point_to_money"    => $helper->getPointToMoney($order->getStoreId()),
                    "status"            => $status,
